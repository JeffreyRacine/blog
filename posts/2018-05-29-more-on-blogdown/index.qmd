...
Furthermore, we can create mathematical expressions and equations using standard $\rm\LaTeX$ commands:
\begin{equation}
\hat\beta=(X'X)^{-1}X'Y
\end{equation}
...
```{r testfig,echo=FALSE,fig.align='center'}
#| label: fig-testfig
#| fig-cap: "Kernel Density Estimator Example."
## Modified by adding the above in order for Quarto Markdown to produce proper reference
set.seed(42)
plot(density(rchisq(10000,df=5)),main="")
```

We can also automatically cross-reference this figure using bookdown cross-referencing extensions, e.g., @fig-testfig.

As well, we can use Bib$\rm\TeX$ for references using bookdown reference syntax, as in @bookdown and @blogdown, with Bib$\rm\TeX$ being seamlessly invoked and the references being automatically generated and added to the end of this post.
